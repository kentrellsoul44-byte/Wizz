...
export const ChatInput: React.FC<ChatInputProps> = ({ 
  ...
  const addFiles = async (files: FileList | File[]) => {
    ...
    // Auto-detect multi-timeframe if appropriate
    if (!isMultiTimeframeMode && !autoMultiTimeframe) {
      const allFiles = [...imageFiles, ...newFiles];
      if (allFiles.length >= 2) {
        const detectionService = new MultiTimeframeDetectionService();
        const isMultiTimeframeDetected = await detectionService.detectMultiTimeframeFromFiles(allFiles);
        if (isMultiTimeframeDetected) {
            ...
            {isUltraMenuOpen && (
                <div className="absolute bottom-full left-0 mb-2 w-72 bg-sidebar-bg border border-border-color rounded-lg shadow-xl z-10 p-3">
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-text-secondary">Advanced analysis with SMC, Patterns & Progressive loading</p>
                            </div>
                    ...
                    <p className="text-xs text-text-secondary mt-2 border-t border-border-color pt-2">
                        {isUltraMode
                            ? 'Ultra mode combines advanced analysis techniques including Smart Money Concepts, pattern recognition, progressive loading, and enhanced confidence calibration for maximum accuracy.'
